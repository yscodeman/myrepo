                   ...
                   unsigned int inputlen)
{
    ...
    if (inputlen >= partlen) {
        ...
        chintMD5Transform(context->state, context->buffer);

        for (i = partlen; i + 64 <= inputlen; i += 64)
            chintMD5Transform(context->state, &input[i]);
...
void chintMD5Final(CHINT_MD5_CTX * context, unsigned char digest[16])
{
    ...
    chintMD5Encode(bits, context->count, 8);
    chintMD5Update(context, PADDING, padlen);
    chintMD5Update(context, bits, 8);
    chintMD5Encode(digest, context->state, 16);
}
